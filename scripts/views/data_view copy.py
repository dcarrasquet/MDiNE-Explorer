###### Save data_view of June 06 2024. Working data_view. Trying to adopt style display: none instead of creating elements to avoid erros during callbacks
...
def check_intervals(interval_cov,interval_taxa):
    global info_current_file
    ...
    return output_cov,info_cov,output_taxa,info_taxa,options_check_boxes,options_check_boxes,options_check_boxes,options_check_boxes,[],[],[],[]
...
@app.callback(Output("interval-covariate-output", 'children'),
              ...
              Output("check-filter-deviation-mean","value"),
                ...
                State('interval-covariate-status', 'data'),
                State('interval-taxa-status', 'data'))
def on_data(ts_cov,ts_taxa,data_cov,data_taxa):
    global info_current_file
    ...
    if data_cov.get('value',None)==None or data_taxa.get('value',None)==None:
        ...
        return check_intervals(data_cov["value"],data_taxa["value"])
...
### Checkboxs to discover filters ###

@app.callback(
    Output('select-filter-columns-zero', 'children'),
    Output("filtered-data-zeros",'children',allow_duplicate=True),
    Input('check-filter-columns-zero', 'value'),prevent_initial_call=True)
def update_output_data(value):
    if len(value)!=0:
        return html.Div(style={'display': 'inline-block','vertical-align': 'middle'},children=[
        ...
        #html.Button('Confirm', id='filter-columns-zero-button', n_clicks=0,style=button_style),
...
@app.callback(
    Output('select-filter-deviation-mean', 'children'),
    Output("filtered-data-dev-mean",'children',allow_duplicate=True),
    Input('check-filter-deviation-mean', 'value'),prevent_initial_call=True
)
def update_output_taxa(value):
    if len(value)!=0:
        nb_taxa=info_current_file["df_taxa"].shape[1]
        return html.Div(style={'display': 'inline-block','vertical-align': 'middle'},children=[
          html.H5("Select the number of colums to keep ",style={"text-indent": '30px','display': 'inline-block'}),
          dcc.Store(id='filter-deviation-mean-status', storage_type=type_storage),
    dcc.Input(id='filter-deviation-mean-input',type='number',min=1,max=nb_taxa,step=1,value=math.floor(nb_taxa/2),persistence=True,persistence_type=type_storage,style=input_field_number_style),
    #html.Button('Confirm', id='filter-deviation-mean-button', n_clicks=0,style=button_style),
    html.Div(id='filter-deviation-mean-output',style={'display': 'inline-block'}),
    ]),None
    else:
        if info_current_file['filter_zeros']!=None:
            info_current_file["df_taxa"]=filter_percent_zeros(info_current_file,info_current_file['filter_zeros'])
        elif info_current_file['taxa_start']!=None:
...
# output the stored clicks in the table cell.
@app.callback(Output("filter-columns-zero-output", 'children'),
              Output("filtered-data-zeros",'children',allow_duplicate=True),
                Input('filter-columns-zero-status', 'modified_timestamp'),
                State('filter-columns-zero-status', 'data'),prevent_initial_call=True)
def on_data(ts, data):
    global info_current_file
    if ts is None:
        raise PreventUpdate
    ...
    if data.get('value',None)==None:
        ...
        df_filtered=filter_percent_zeros(info_current_file,data["value"])
        info_current_file["df_taxa"]=df_filtered
        info_current_file["filter_zeros"]=data["value"]
        nb_taxa=info_current_file['taxa_end']-info_current_file['taxa_start']+1
        message=html.H5(f"Filter with pourcent of zeros: {nb_taxa-df_filtered.shape[1]} taxa deleted",style={"text-indent": '15px'})
        return [html.H5(f"{nb_taxa-df_filtered.shape[1]} taxa deleted",style={'display': 'inline-block'})],[message]
...
# if options[0]["value"]=="":
#         print("Je suis nzfebikdb")
#         info_current_file["filter_zeros"]=None

# State("check-filter-columns-zero","options"),

# output the stored clicks in the table cell.
@app.callback(Output("filter-deviation-mean-output", 'children'),
...
def on_data(check_value,ts, data_dev,check_filter_dev):
    global info_current_file
    if ts is None:
        raise PreventUpdate


    data_dev = data_dev or {}

    if data_dev.get('value',None)==None:
        return None,None
    elif check_filter_dev==[]:
        return None,None
    else:
        #if info_current_file["filter_zeros"]==None:
        if check_value==[]:
            ...
            info_current_file["df_taxa"]=get_df_file(info_current_file).iloc[:,info_current_file["taxa_start"]-1:info_current_file["taxa_end"]]
            nb_taxa=info_current_file['taxa_end']-info_current_file['taxa_start']+1
            df_filtered=filter_deviation_mean(info_current_file,data_dev["value"])
            info_current_file["df_taxa"]=df_filtered
            filter_dev_mean_output=html.H5(f"{nb_taxa-df_filtered.shape[1]} taxa deleted",style={'display': 'inline-block'})
            message=html.H5(f"Filter with deviation/mean ratio: {nb_taxa-df_filtered.shape[1]} taxa deleted",style={"text-indent": '15px'})
        else:
            df_filtered=filter_percent_zeros(info_current_file,info_current_file["filter_zeros"])
            info_current_file["df_taxa"]=df_filtered
            nb_taxa=info_current_file["df_taxa"].shape[1]
            df_filtered=filter_deviation_mean(info_current_file,data_dev["value"])
            info_current_file["df_taxa"]=df_filtered
        ...
        return filter_dev_mean_output,message
...
# # add a click to the appropriate store.
# @app.callback(Output('filter-columns', 'data'),
#                 Input("filter-columns-zero-input","value"),
#                 Input("filter-deviation-mean-input","value"),
#                 State('filter-columns', 'data'))
# def on_click(value_zero,value_dev, data):
#     # if n_clicks is None:
#     #     # prevent the None callbacks is important with the store component.
#     #     # you don't want to update the store for nothing.
#     #     raise PreventUpdate
#     if value_dev is None and value_zero is None:
#         raise PreventUpdate

#     # Give a default data dict with 0 clicks if there's no data.
#     data = data or {'value_zero': "",'value_dev': ""}

#     if value_dev!=None:
#         data['value_dev']=value_dev
#     if value_zero!=None:
#         data['value_zero']=value_zero
...
### Méthode de ChatGPT
# @app.callback(
#     Output('filter-columns', 'data'),
#     [Input("filter-columns-zero-input", "value"),
#      Input("filter-deviation-mean-input", "value")],
#     [State('filter-columns', 'data')]
# )
# def on_click(value_zero, value_dev, data):
#     # Get the trigger context to determine which input triggered the callback
...
#     # Update the data dict based on which inputs are present
#     if 'filter-deviation-mean-input.value' in inputs:
#         data['value_dev'] = value_dev
#     if 'filter-columns-zero-input.value' in inputs:
#         data['value_zero'] = value_zero
...
#                 Input('filter-columns', 'modified_timestamp'),
#                 State('filter-columns', 'data'))
# def on_data(ts, data):
#     global info_current_file
#     if ts is None:
...
#     if data.get('value_dev',None)==None and data.get('value_zero',None)==None:
#         return None,None,None
#     else:
        # if data.get('value_zero',None)!=None:
        #     #print(data["value_zero"],type(data["value_zero"]))
        #     df_filtered=filter_percent_zeros(info_current_file,data["value_zero"])
        #     info_current_file["filter_zeros"]=data["value_zero"]
        #     info_current_file["df_taxa"]=df_filtered
        #     nb_taxa=info_current_file['taxa_end']-info_current_file['taxa_start']+1
        #     #filter_zero_output=html.H5(f"{nb_taxa-df_filtered.shape[1]} deleted taxa ",style={'display': 'inline-block'})
        #     infos_filtered_data.append(html.H5(f"Filter with pourcent of zeros: {nb_taxa-df_filtered.shape[1]} taxa deleted",style={"text-indent": '15px'}))
        # if data.get('value_dev',None)!=None:
        #     if data.get('value_zero',None)==None:
        #         nb_taxa=info_current_file['taxa_end']-info_current_file['taxa_start']+1
        #     else:
        #         nb_taxa=info_current_file["df_taxa"].shape[1]
        #     df_filtered=filter_deviation_mean(info_current_file,data["value_dev"])
        #     info_current_file["df_taxa"]=df_filtered
        #     #filter_dev_mean_output=html.H5(f"{nb_taxa-df_filtered.shape[1]} deleted taxa ",style={'display': 'inline-block'})
        #     infos_filtered_data.append(html.H5(f"Filter with deviation/mean ratio: {nb_taxa-df_filtered.shape[1]} taxa deleted",style={"text-indent": '15px'}))
            
        # return infos_filtered_data
